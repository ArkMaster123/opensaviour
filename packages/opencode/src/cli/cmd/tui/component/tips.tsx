...
const TIPS = [
  ...
  "Create {highlight}opensaviour.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/opensaviour/opensaviour.json{/highlight} for global config",
  ...
  "OpenSaviour auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.opensaviour/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.opensaviour/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "OpenSaviour auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "OpenSaviour uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files to {highlight}.opensaviour/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.opensaviour/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenSaviour from reading sensitive files",
  "Use {highlight}opensaviour run{/highlight} for non-interactive scripting",
  "Use {highlight}opensaviour run --continue{/highlight} to resume the last session",
  "Use {highlight}opensaviour run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}opensaviour serve{/highlight} for headless API access to OpenSaviour",
  "Use {highlight}opensaviour run --attach{/highlight} to connect to a running server",
  "Run {highlight}opensaviour upgrade{/highlight} to update to the latest version",
  "Run {highlight}opensaviour auth list{/highlight} to see all configured providers",
  "Run {highlight}opensaviour agent create{/highlight} for guided agent creation",
  "Use {highlight}/opensaviour{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}opensaviour github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opensaviour fix this{/highlight} on issues to auto-create PRs",
